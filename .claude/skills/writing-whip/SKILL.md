...
This file governs generation. Two neighbours cover the rest of the pipeline and read this one. Invoke them with the Skill tool where it exists (Codex and Zed: read the linked file), and do it as part of the task rather than as a suggestion:

- [humanize](../humanize/SKILL.md) is the edit-time pass over text that already exists. Invoke it whenever the request is to clean up, de-slop, or humanize a text the user hands over; this file has no triage, no risk tiers and no greps of its own. When a humanize rewrite produces new sentences, those are written under this file, so a rewrite never reintroduces a behaviour listed above. Its tells.md carries the same list as the Behaviours section here, with currency tags and before/after tables.
- [write-in-my-voice](../write-in-my-voice/SKILL.md) drafts as Magnus. Invoke it whenever the text goes out in his name (a message, email, post or article he will send or publish). It loads this file itself and then layers his voice profiles on top. Where the two disagree, the voice profile wins on the points it names explicitly (a real rule of three, a rhetorical question, second-person "du", parenthetical glosses of English terms, sparing intensifiers, one-sentence paragraphs on LinkedIn). Everything else here still applies.

Run order for a piece of prose in his name: this file, then write-in-my-voice, then a narrow humanize pass on the result.
